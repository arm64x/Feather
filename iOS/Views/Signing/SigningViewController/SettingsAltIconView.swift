...
struct SettingsAltIconView: View {
    ...
    var body: some View {
        NavigationView {
            ScrollView {
                let alternateIcons = loadAlternateIcons()
                
                if alternateIcons.isEmpty {
                    VStack(spacing: 20) {
                        Text(NSLocalizedString("APP_SIGNING_VIEW_CONTROLLER_CELL_NO_ALT_ICONS_AVAILABLE", comment: ""))
                            .font(.headline)
                    ...
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 100), spacing: 8)], spacing: 8) {
                        ...
                        ForEach(alternateIcons.sorted(by: { $0.key < $1.key }), id: \.key) { name, path in
                            IconButton(
                                iconPath: path,
                                name: name,
                                applicationPath: applicationPath,
